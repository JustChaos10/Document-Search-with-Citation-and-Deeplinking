...
def resolve_embeddings(
    ...
    if not (HAS_LC_HF and HuggingFaceEmbeddings is not None):
        ...
        return _google_embeddings_fallback(embedding_api_key, embedding_model_name)
    ...
    return _FailoverEmbeddings(
        ...
        fallback_factory=lambda: _google_embeddings_fallback(embedding_api_key, embedding_model_name),
    )


def _google_embeddings_fallback(embedding_api_key: str, requested_model: str) -> object:
    ...
    if not embedding_api_key:
        raise ValueError(
            "EMBEDDING_API_KEY (or the GPU provider key) is required for Google embeddings fallback."
        )
    return GoogleGenerativeAIEmbeddings(model=fallback_model, google_api_key=embedding_api_key)
